...
class _OtpTextFieldState extends State<OtpTextField> {
        ...
        onKey: (RawKeyEvent event) {
          if (event is RawKeyDownEvent) {
            if (event.data is RawKeyEventDataAndroid) {
              ...
              if (data.keyCode == 67) {
                // 67 is the key code for the backspace key on Android
                print('Backspace key pressed');
                _verificationCode[index-1]= "";
                FocusScope.of(context).requestFocus(_focusNodes[index - 1]);
                _textControllers[index-1]?.value =TextEditingValue.empty;
                print(_verificationCode);
              } else {
                // if (_focusNodes[index]) {
                  print("Test key board ${data.keyLabel}");
                  print(_verificationCode);
                  // FocusScope.of(context).requestFocus(_focusNodes[index + 1]);
                  // _focusNodes[index].value = Number(data.keyLabel)
                // }
              }
            } else if (event.data is RawKeyEventDataLinux) {
              final data = event.data as RawKeyEventDataLinux;
              if (data.keyLabel == 'Backspace') {
                print('Backspace key pressed');
              }
            }
          }
  ...
  void changeFocusToNextNodeWhenValueIsEntered({
    ...
    print("Xuat ra ${value} ${indexOfTextField}");
    ...
    if (value.length > 0) {
      ...
      if(_verificationCode[indexOfTextField] != "" && _verificationCode[indexOfTextField] != null){
        FocusScope.of(context).requestFocus(_focusNodes[indexOfTextField + 1]);
      }
    }
  }
