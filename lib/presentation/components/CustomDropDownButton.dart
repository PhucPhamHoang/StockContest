...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  double getItemOffset(int index) {
    double offset = kMaterialListPadding.top - 50;
...
class DropdownButtonCustom<T> extends StatefulWidget {
  ...
  DropdownButtonCustom._formField({
    super.key,
    required this.items,
    this.selectedItemBuilder,
    this.value,
...
class _DropdownButtonCustomState<T> extends State<DropdownButtonCustom<T>>
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.items!
        .where(
            (DropdownMenuItemCustom<T> item) => item.value == widget.value)
        .length ==
        1);
        ...
        builder: (FormFieldState<T> field) {
          ...
          return Focus(
            ...
            child: Builder(builder: (BuildContext context) {
              return DropdownButtonHideUnderlineCustom(
                child: DropdownButtonCustom<T>._formField(
                  items: items,
