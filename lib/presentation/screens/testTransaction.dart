...
class _TransactionContestScreenState extends State<TransactionContestScreen> {
  ...
  GlobalKey<AppDropDownButtonState<String>> subAccountKey =
      GlobalKey<AppDropDownButtonState<String>>();
  GlobalKey<AppDropDownButtonState<ContestAccountInfoModel>> contestKey =
      GlobalKey<AppDropDownButtonState<ContestAccountInfoModel>>();
  String? competitionCode;
  ContestAccountInfoModel? currentContest;

  @override
  void initState() {
    super.initState();
    context.read<ContestAccountInfoBloc>().add(ContestAccountInfoEvent.fetch());
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
        providers: [
          ...
          //       context.read<ContestAccountInfoRepository>()),
          // ),
          // BlocProvider(
          //   create: (context) =>
        ...
        child: AppLayout(
            ...
            body: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              BlocBuilder<ContestAccountInfoBloc, ContestAccountInfoState>(
                  builder: (context, state) {
                state.maybeMap(
                    success: (state) {
                      currentContest = state.data.listData?.firstOrNull;
                    },
                    orElse: () {});
                return Row(
                  children: [
                    Expanded(
                      child: Column(
                        children: [
                          ...
                          AppDropDownButton<ContestAccountInfoModel>(
                            key: contestKey,
                            onChange: (data) {
                              subAccountKey.currentState?.setValue(null);
                              if (data != null && data.data != null) {
                                currentContest = data.data;
                              }
                              changeTab(context, currentTap,
                                  data?.data.subAccountId.toString());
                              return false;
                            },
                            listData: state.data.listData
                                    ?.map((e) =>
                                        DropDownData<ContestAccountInfoModel>(
                                            text: e.challengeCodeId, data: e))
                                    .toList() ??
                                [],
                          ),
                        ],
  ...
  Widget buildTabBar() {
    return Builder(builder: (context) {
      return AppCupertinoTabBar(
          ...
          foregroundColor: AppColor.orangeDark,
          widgets: [
            Center(
              child: Text(
                'Đặt lệnh',
                      ...
                      Expanded(
                        child: Row(
                          children: [
                            ...
                            Text(
                                state.data.stock?.ceiling.toStringAsSeparator ??
                                    '-',
                      ...
                      Expanded(
                        child: Row(
                          children: [
                            ...
                            Text(
                              state.data.stock?.ref.toStringAsSeparator ?? '-',
                              style: AppStyle.medianRegular.copyWith(
                                  color: state.data.stock != null
                                      ? AppColor.yellowDark
                                      : AppColor.white),
                ...
                child: Column(
                  children: [
                    ...
                    CardList(
                      widgets: [
                        ...
                        CardItem.normal(
                            ...
                            valueStyle: AppStyle.largeRegular
                                .copyWith(color: contestAsset.rateProfitPctColor)),
                      ],
                    ),
                    const SizedBox(
                      height: 30,
                    ...
                    BlocBuilder<ContestRankBloc, ContestRankState>(
                      builder: (context, state) {
                        int? rank = state.data.myRank?.ranking;
                        return CardList(
                          widgets: [
                            CardItem.normal(
                              title: 'Thứ hạng',
                    ...
                    CardList(
                      widgets: [
                        ...
                        CardItem.extend(
                          title: 'Tổng giá trị chứng khoán',
                                ...
                                Navigator.of(context)
                                    ...
                                    builder: (_) =>
                                        ContestOrderDetail(
                                          bloc: context.read<
                                              ContestOrderBloc>(),
                                          userid: currentContest?.subAccountId.first ??"",
                                        )))
                                    .then((value) => context
                                    .read<ContestOrderBloc>()
                                    .currentOrder = null);
